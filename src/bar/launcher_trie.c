...
void launcher_trie_add(launcher_trie_node_t **tree, const char *word,
                       size_t length) {
  ...
  size_t split;
  if(*tree == NULL ||
     ((*tree)->length <= LAUNCHER_TRIE_NODE_ARRAY_SIZE &&
      (*tree)->value.array[0] > word[0]) ||
     ((*tree)->length > LAUNCHER_TRIE_NODE_ARRAY_SIZE &&
      (*tree)->value.pointer[0] > word[0])) {
    t = *tree;
  ...
  for(size_t i = 0;;) {
    while((curr->length <= LAUNCHER_TRIE_NODE_ARRAY_SIZE &&
      ...
      if(curr->adjacent == NULL ||
        ...
        if(length - i <= LAUNCHER_TRIE_NODE_ARRAY_SIZE) {
          memcpy(curr->adjacent->value.array, word + i,
                 (length - i) * sizeof(char));
        } else {
          curr->adjacent->value.pointer = malloc((length - i) * sizeof(char));
          memcpy(curr->adjacent->value.pointer, word + i,
                 (length - i) * sizeof(char));
        }
    ...
    if(i == length) {
      ...
      if((curr->next->length <= LAUNCHER_TRIE_NODE_ARRAY_SIZE &&
        ...
        curr->next = malloc(sizeof(launcher_trie_node_t));
        if(length - i <= LAUNCHER_TRIE_NODE_ARRAY_SIZE) {
          memcpy(curr->next->value.array, word + i,
                 (length - i) * sizeof(char));
        } else {
          curr->next->value.pointer = malloc((length - i) * sizeof(char));
          memcpy(curr->next->value.pointer, word + i,
                 (length - i) * sizeof(char));
        }
        curr->next->end = true;
...
void launcher_trie_unmark(launcher_trie_node_t *node) {
  ...
  while(curr != NULL) {
    if(curr->next != NULL) launcher_trie_unmark(curr->next);
    curr = curr->adjacent;
  }
}
...
CONST launcher_trie_search_node_t *launcher_trie_search(
  ...
  size_t j;
  for(size_t i = 0;;) {
    while(curr != NULL && ((curr->length <= LAUNCHER_TRIE_NODE_ARRAY_SIZE &&
                            curr->value.array[0] != word[i]) ||
                           (curr->length > LAUNCHER_TRIE_NODE_ARRAY_SIZE &&
                            curr->value.pointer[0] != word[i])))
      curr = curr->adjacent;
...
void launcher_trie_10_shortest(const launcher_trie_node_t *node,
...
void launcher_trie_10_shortest_sr(

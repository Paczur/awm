...
static void hint_window_update_wm_state(xcb_window_t window, WINDOW_STATE prev,
                                        WINDOW_STATE state) {
  if(prev == state && state < 0) return;
  uint32_t st[] = {(state == WINDOW_ICONIC)               ? 3
                   : ((size_t)state == workspace_focused) ? 1
                                                          : 0,
                   XCB_NONE};
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, window, WM_STATE, WM_STATE,
                      32, 2, &st);
}
...
static void hint_window_update_net_wm_desktop(xcb_window_t window,
                                              WINDOW_STATE prev,
                                              WINDOW_STATE state) {
  if((prev >= 0) == (state >= 0)) return;
  if(state >= 0) {
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, window, _NET_WM_DESKTOP,
...
static void hint_window_update_net_client_list(xcb_window_t window,
                                               WINDOW_STATE prev,
                                               WINDOW_STATE state) {
  if((prev == WINDOW_WITHDRAWN) == (state == WINDOW_WITHDRAWN)) return;
...
static void hint_window_update_net_wm_allowed_actions(xcb_window_t window,
  ...
  if(prev < 0) {
    ...
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, window,
                        _NET_WM_ALLOWED_ACTIONS, XCB_ATOM_ATOM, 32, 2, actions);
  } else {
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, window,
                        _NET_WM_ALLOWED_ACTIONS, XCB_ATOM_ATOM, 32, 1, actions);
  }
}
...
static void hint_window_update_net_wm_state(xcb_window_t window,
  ...
  if(prev == WINDOW_ICONIC) {
    ...
    for(size_t i = 0; i < size; i++) {
      if(atoms[i] == _NET_WM_STATE_HIDDEN) {
        ...
        if(size - 1 == 0) {
          ...
          xcb_change_property(conn, XCB_PROP_MODE_REPLACE, window,
                              _NET_WM_STATE, XCB_ATOM_ATOM, 32, size - 1,
                              atoms);
        }
        break;
      }
    ...
    xcb_change_property(conn, XCB_PROP_MODE_APPEND, window, _NET_WM_STATE,
                        XCB_ATOM_ATOM, 32, 1,
...
bool hint_atom_frame_extents(xcb_atom_t a) {
...
bool hint_window_urgent(xcb_window_t win, xcb_atom_t atom) {
  ...
  if(atom == WM_HINTS) {
    ...
    cookie = xcb_get_property(conn, 0, win, _NET_WM_STATE, XCB_ATOM_ATOM, 0,
                              MAX_WINDOW_STATE_ATOMS);
    reply = xcb_get_property_reply(conn, cookie, NULL);
...
void hint_frame_extents_set(xcb_window_t window) {
  ...
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, window, _NET_FRAME_EXTENTS,
...
void hint_workspace_focused_set(size_t workspace) {
  ...
  xcb_change_property(conn, XCB_PROP_MODE_REPLACE, screen->root,
                      _NET_CURRENT_DESKTOP, XCB_ATOM_CARDINAL, 32, 1,
                      &workspace_focused);
#define PRINT OUT(workspace);
  LOGF(HINT_TRACE);
#undef PRINT
...
#define PRINT              \
  OUT(window);             \
  OUT_WINDOW_STATE(prev);  \
  OUT_WINDOW_STATE(state); \
  OUT(workspace_focused);  \
  OUT_ARR(client_list, client_list_length);
  LOGF(HINT_TRACE);
#undef PRINT
}

...
static void c_bar_update_minimized(void) {
  bar_update_minimized((const plist_t*)layout_get_minimized(),
                       offsetof(window_t, name));
}
static void c_bar_update_workspace(void) {
  bar_update_workspace(layout_get_focused_workspace(),
                       layout_workspace_empty);
}
...
void c_run(const char* cmd) { sh(cmd); }
...
//TODO: Cleanup useless event handlers in layout
...
void c_init(void) {
  ...
  bar_init_t binit = (bar_init_t){
    conn, screen, visual_type, .bar_containers = t_rect,
      .bar_container_count = monitor_count, CONFIG_BAR_COMPONENT_PADDING,
      ...
      { CONFIG_BAR_INFO_MIN_WIDTH,
        ...
        LENGTH((block_info_data_t[])CONFIG_BAR_INFO_BLOCKS), empty, shout },
